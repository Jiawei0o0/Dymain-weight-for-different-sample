...
class RegionLoss_3D_weight(nn.Module):
    # https://github.com/Youmin-Kim/GLD/blob/main/gld.py
    def __init__(self, spatial_size, pool='Avg'):
        super(RegionLoss_3D_weight, self).__init__()
    ...
    def mse_loss_weight_3d(self, input1, input2, weight):
        loss = (input1 - input2)**2
        loss = torch.mean(loss, dim=(1, 2)) * weight
        return torch.sum(loss)

    def forward(self, p1, p2, weight, dis_f='MSE'):
        ...
        sim_a = torch.cosine_similarity(aa.unsqueeze(2), aa.unsqueeze(1), dim=-1)
        sim_b = torch.cosine_similarity(bb.unsqueeze(2), bb.unsqueeze(1), dim=-1)

        if dis_f == 'MSE':
            diff = self.mse_loss_weight_3d(sim_a, sim_b, weight)
        else:
            diff = nn.L1Loss()(sim_a, sim_b)
