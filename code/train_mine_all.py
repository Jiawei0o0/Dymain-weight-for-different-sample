...
# self param
parser.add_argument('--dice', type=str, default='B', help='A: 1-dice B: dice')
parser.add_argument('--spatial_size', type=int, default=5, help='spatial size')

args = parser.parse_args()

snapshot_path = "/mnt/imtStu/jwsu/NewWeightSample/{}_{}_{}_dice_{}_spatial_{}_labeled_all_v2_0113/{}".format(
    args.dataset_name, args.exp, args.dice, args.spatial_size, args.labelnum, args.model)
...
if __name__ == "__main__":
    ...
    consistency_criterion = losses.mse_loss_weight_3d
    dice_loss = losses.Binary_dice_loss
    region_loss = losses.RegionLoss_3D(spatial_size=args.spatial_size).cuda()
    weight_criterion = losses.Binary_dice_loss_weight
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(num_outputs - 2):
                for j in range(num_outputs - 2):
                    if i != j:
                        weight_matrix = 1 - weight_criterion(y_ori[i], y_ori[j])
                        loss_consist += consistency_criterion(y_ori[i], y_ori[j], weight_matrix)

            iter_num = iter_num + 1
            consistency_weight = get_current_consistency_weight(iter_num // 150)

            loss_region = region_loss(outputs[0], outputs[1])

            loss = args.lamda * loss_seg_dice + consistency_weight * (loss_consist + loss_region)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if iter_num >= 800 and iter_num % 200 == 0:
                ...
                snapshot_img = torch.zeros(size=(D, 3, (num_outputs + 2) * H + (num_outputs + 2) * ins_width,
                                                 W + ins_width),
                                           dtype=torch.float32)
                ...
                snapshot_img[:, 0, :H, :W] = (train_img - torch.min(train_img)) / (torch.max(train_img) -
                                                                                   torch.min(train_img))
                snapshot_img[:, 1, :H, :W] = (train_img - torch.min(train_img)) / (torch.max(train_img) -
                                                                                   torch.min(train_img))
                snapshot_img[:, 2, :H, :W] = (train_img - torch.min(train_img)) / (torch.max(train_img) -
                                                                                   torch.min(train_img))
                ...
                for idx in range(num_outputs):
                    ...
                    snapshot_img[:, 0, begin * H + begin * ins_width:end * H +
                                 begin * ins_width, :W] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
                    snapshot_img[:, 1, begin * H + begin * ins_width:end * H +
                                 begin * ins_width, :W] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
                    snapshot_img[:, 2, begin * H + begin * ins_width:end * H +
                                 begin * ins_width, :W] = y_ori[idx][labeled_bs:][0, 1].permute(2, 0, 1)
