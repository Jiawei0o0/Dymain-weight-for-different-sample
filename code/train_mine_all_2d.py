...
parser.add_argument('--root_path', type=str, default='/home/jwsu/semi/data/ACDC', help='Name of Experiment')
...
def multi_dice_loss(score, target):
    class_wise_dice = torch.zeros(score.shape[0])
    for i in range(score.shape[0]):
        class_wise_dice[i] = _dice_loss(score[i], target[i])
...
def patients_to_slices(dataset, patiens_num):
    ref_dict = None
    if "ACDC" in dataset:
        ref_dict = {"3": 68, "7": 136, "14": 256, "21": 396, "28": 512, "35": 664, "70": 1312}
    elif "Prostate":
...
def train(args, snapshot_path):
    ...
    weight_criterion = losses.DiceLoss_weight()
    consistency_criterion = losses.mse_loss_weight

    region_loss = losses.RegionLoss_2D(spatial_size=args.spatial_size).cuda()
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            for idx in range(num_outputs-2):
                ...
                # note:softmax == sharpening(softmax)
                y_all = outputs[idx]
            ...
            loss_region = region_loss(outputs[2], outputs[3])
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for _, sampled_batch in enumerate(valloader):
                    metric_i = val_2d.test_single_volume(sampled_batch["image"],
                                                         sampled_batch["label"],
                                                         model,
                                                         classes=num_classes)
...
if __name__ == "__main__":
    ...
    snapshot_path = "/mnt/imtStu/jwsu/WeightSample/ACDC_all_{}_{}_labeled/{}".format(
